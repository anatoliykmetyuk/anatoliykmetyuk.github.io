...
```scala
println(flightsDatabase.filter(f => f._1 == "Geneva"))
```

Outputs:

```
List((Geneva,Vienna,200), (Geneva,Zurich,100))
```
...
In our example, the predicate is `f => f._1 == "Geneva"`. It is understood as follows: "if `f` is a flight from our list, allow that flight in the resulting collection if its departure city equals `"Geneva"`. The `==` means equality in Scala, so `a == b` means, "check if `a` equals `b`" in the context of such predicates.
...
![](/scala-intro_assets/07-flights-search-engine-logic/CleanShot%202022-08-18%20at%2022.09.25@2x.png)
...
```
List((Geneva,Vienna,200), (Vienna,London,150), (London,Geneva,300), (Geneva,Zurich,100), (Vienna,London,300), (Vienna,Helsinki,200))
```
...
```scala
flightsDatabase.foreach(f => println(s"Departure: ${f._1}; Destination: ${f._2}; Cost: ${f._3}"))
```

Output:

```
Departure: Geneva; Destination: Vienna; Cost: 200
Departure: Vienna; Destination: London; Cost: 150
Departure: London; Destination: Geneva; Cost: 300
Departure: Geneva; Destination: Zurich; Cost: 100
Departure: Vienna; Destination: London; Cost: 300
Departure: Vienna; Destination: Helsinki; Cost: 200
```

There's a catch though: we interpolate the parts of the tuple into a string using the `${f._1}` syntax, and not `$f._1` as you would expect. Whenever we interpolate something that's accessed using an ownership syntax `.`, we need to enclose that something in between curly braces as in the code above.
...
![](/scala-intro_assets/07-flights-search-engine-logic/CleanShot%202022-08-19%20at%2016.54.46@2x.png)
...
```scala
println(flightsDatabase.filter(f => f._1 == "Geneva").map(f => f._2))
```
...
We want the connecting flights in the format of tuples consisting of two pieces of data: the first leg of the flight and the second leg of the flight, as follows (yes, tuples can be nested!):

```
((Geneva,Vienna,200), (Vienna,London,150))
```
...
val genevaConnectingFlights = flightsDatabase
  .filter(f => f._1 == "Geneva")
  .map { firstLeg =>
    val possibleSecondLegs = flightsDatabase.filter(f => f._1 == firstLeg._2)
    (firstLeg, possibleSecondLegs)
  }
println(genevaConnectingFlights)
```

Output:

```
List(((Geneva,Vienna,200),List((Vienna,London,150), (Vienna,London,300), (Vienna,Helsinki,200))), ((Geneva,Zurich,100),List()))
```

First of all, there's a lot going on in the code that we've never seen before:

- We are storing the computation of our result in a separate variable. We don't write it all as an argument to `println` because it's easier to read this way.
- `filter` and `map` are called from the new line with an indentation. This notation does exactly the same as before (e.g. `flightsDatabase.filter(...).map(...)` all in one line) and is also for readability.
...
![](/scala-intro_assets/07-flights-search-engine-logic/CleanShot%202022-08-19%20at%2017.20.09@2x.png)
...
```
List(List(((Geneva,Vienna,200),(Vienna,London,150)), ((Geneva,Vienna,200),(Vienna,London,300)), ((Geneva,Vienna,200),(Vienna,Helsinki,200))), List())
```
...
```
List(((Geneva,Vienna,200),(Vienna,London,150)), ((Geneva,Vienna,200),(Vienna,London,300)), ((Geneva,Vienna,200),(Vienna,Helsinki,200)))
```

> 🎯**Exercise**: use `foreach` to print every pair of connecting flights nicely, so that the end user of your search engine doesn't see the parentheses or the `List`.
...
```scala
flightsDatabase.sortBy(f => f._3).foreach(f => println(f))
```

Outputs:

```
List((Geneva,Zurich,100), (Vienna,London,150), (Geneva,Vienna,200), (Vienna,Helsinki,200), (London,Geneva,300), (Vienna,London,300))
```

And if we want to reverse the order, we can call the `reverse` method on the resulting list:

```scala
flightsDatabase.sortBy(f => f._3).reverse.foreach(f => println(f))
```
